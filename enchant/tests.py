...
class TestPy3Import(unittest.TestCase):
    def test_py3_import(self):
        if sys.platform == "darwin" and sys.version_info[0] >= 3:
            import importlib
            import enchant
            enchant_finder = importlib.find_loader('enchant') is not None
            self.assertTrue(enchant_finder)
...
def buildtestsuite(recurse=True):
    ...
    suite.addTest(unittest.makeSuite(TestPy3Import))
    return suite
